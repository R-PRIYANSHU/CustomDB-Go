...
func mmapInit(fp *os.File) (int, []byte, error) {
    ...
    if fi.Size() % BTreeImplement.BTREE_PAGE_SIZE != 0 {
        return 0, nil, errors.New("File size is not a multiple of page size")
    }

    mmapSize := 64 << 20
    Utils.Assert(mmapSize % BTreeImplement.BTREE_PAGE_SIZE == 0)
...
func extendMmap(db *KV, npages int) error {
    if db.mmap.total >= npages * BTreeImplement.BTREE_PAGE_SIZE {
        return nil
    }
...
func extendFile(db *KV, npages int) error {
    filePages := db.mmap.file / BTreeImplement.BTREE_PAGE_SIZE
    if filePages >= npages {
        return nil
    }
    ...
    fileSize := filePages * BTreeImplement.BTREE_PAGE_SIZE
